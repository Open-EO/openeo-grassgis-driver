...
def create_process_description():
    ...
    rv = ReturnValue(description="`true` if `x` is strictly less than `y`, `null` if any operand is `null`, otherwise `false`.",
                     schema={
                             "type": [
                               "boolean",
                               "null"
    ...
    graph = ProcessGraph(title="title", description="description", process_graph={"lt_1": node})
