...
def create_process_description():
    ...
    rv = ReturnValue(description="`true` if `x` is less than or equal to `y`, `null` if any operand is `null`, otherwise `false`.",
                     schema={
                             "type": [
                               "boolean",
                               "null"
    ...
    graph = ProcessGraph(title="title", description="description", process_graph={"lte_1": node})
