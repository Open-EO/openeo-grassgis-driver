...
def create_process_description():
    ...
    rv = ReturnValue(description="The computed sample standard deviation.",
                     schema={
                             "type": [
                               "number",
                               "null"
    ...
    graph = ProcessGraph(title="title", description="description", process_graph={"sd_1": node})
    examples = [ProcessExample(title="Simple example", description="Simple example",
                               process_graph=graph)]

    pd = ProcessDescription(id=PROCESS_NAME,
                            description="Computes the sample standard deviation, which quantifies the amount of variation of an array of numbers.",
